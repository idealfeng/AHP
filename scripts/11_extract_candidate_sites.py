"""提取高适宜区候选点脚本：从适宜性分级图中提取 class = 5 的高适宜区，进行面积过滤，计算统计信息，并输出结果。"""
...
CANDIDATE_DIR = BASE_DIR / "results" / "candidates"
...
# 最小候选区面积，单位 m²
# 100m 分辨率下，一个像元是 10000 m²。
# 这里设为 30000 m²，相当于至少 3 个像元，避免碎斑太多。
MIN_AREA_M2 = 30000
...
def plot_candidate_map(areas, points):
    ...
    points.plot(ax=ax, markersize=12, color="blue")
    ...
    legend_items = [
        ...
        Patch(facecolor="blue", label="候选区边界 / 中心点"),
    ]

    ax.legend(handles=legend_items, loc="lower left", frameon=True, fontsize=8)

    ax.set_title("高适宜区与候选点分布图 / Candidate Sites", fontsize=14)
