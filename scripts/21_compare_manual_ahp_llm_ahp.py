# 对比 manual AHP 和 LLM-AHP 的适宜性结果，分析差异并可视化
...
LLM_SCORE = BASE_DIR / "results" / "suitability" / "llm_ahp" / "suitability_score.tif"
LLM_CLASS = BASE_DIR / "results" / "suitability" / "llm_ahp" / "suitability_class.tif"

COMPARE_DIR = BASE_DIR / "results" / "comparison" / "manual_ahp_vs_llm_ahp"
MAP_DIR = BASE_DIR / "results" / "maps" / "comparison" / "manual_ahp_vs_llm_ahp"

COMPARE_DIR.mkdir(parents=True, exist_ok=True)
MAP_DIR.mkdir(parents=True, exist_ok=True)

SCORE_DIFF_TIF = COMPARE_DIR / "score_difference_llm_minus_manual.tif"
CLASS_DIFF_TIF = COMPARE_DIR / "class_difference_llm_minus_manual.tif"
...
def check_alignment(meta1, meta2):
    ...
    for key in keys:
        if meta1[key] != meta2[key]:
            raise ValueError(f"栅格未对齐：{key} 不一致。")
...
def make_comparison():
    ...
    ahp_score, valid_as, meta_ahp_score, nodata_as, _, _ = read_raster(LLM_SCORE)

    baseline_class, valid_bc, meta_class, nodata_bc, _, _ = read_raster(MANUAL_CLASS)
    ahp_class, valid_ac, meta_ahp_class, nodata_ac, _, _ = read_raster(LLM_CLASS)
...
def plot_score_diff(result):
    ...
    cbar.set_label("LLM-AHP Score - Manual AHP Score")

    ax.set_title("LLM-AHP与Manual AHP综合得分差异图", fontsize=14)
...
def plot_class_diff(result):
    ...
    cbar = plt.colorbar(im, ax=ax, fraction=0.035, pad=0.02)
    cbar.set_label("LLM-AHP Class - Manual AHP Class")

    ax.set_title("LLM-AHP与Manual AHP适宜性等级差异图", fontsize=14)
    ax.set_xlabel("Easting (m)")
    ax.set_ylabel("Northing (m)")
    ax.set_aspect("equal")
...
def plot_high_overlap(result):
    ...
    legend_items = [
        Patch(facecolor=colors[0], label="仅Manual AHP高适宜"),
        Patch(facecolor=colors[1], label="仅LLM-AHP高适宜"),
        Patch(facecolor=colors[2], label="二者共同高适宜"),
    ]
    ax.legend(handles=legend_items, loc="lower left", frameon=True, fontsize=9)

    ax.set_title("Manual AHP与LLM-AHP高适宜区重叠图", fontsize=14)
    ax.set_xlabel("Easting (m)")
    ax.set_ylabel("Northing (m)")
    ax.set_aspect("equal")
...
def main():
    for path in [MANUAL_SCORE, MANUAL_CLASS, LLM_SCORE, LLM_CLASS]:
        if not path.exists():
